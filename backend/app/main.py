...
async def lifespan(app: FastAPI):
    ...
    async with AsyncSessionLocal() as db:
        stuck_recs = (
            (await db.execute(select(Recording).where(Recording.status == 'recording')))
            .scalars()
            .all()
        )
        stuck_cams = (await db.execute(select(Camera).where(Camera.is_recording))).scalars().all()
        ...
        if stuck_recs or stuck_cams:
            await db.commit()
            logger.warning(
                f'启动清理: 重置 {len(stuck_recs)} 条孤立录制记录, {len(stuck_cams)} 台摄像头状态'
            )
    _shutdown_event = None
    if is_packaged():
        from app.desktop import open_browser, run_tray_icon
    ...
    async with AsyncSessionLocal() as db:
        result = await db.execute(select(ScheduleModel).where(ScheduleModel.enabled))
        enabled_schedules = [
            {
                'id': s.id,
    ...
    for sched in enabled_schedules:
        ...
        async def _trigger(mac, sd=sched['segment_duration']):
            ...
            async with AsyncSessionLocal() as _db:
                cam = (
                    await _db.execute(select(Camera).where(Camera.device_mac == mac))
                ).scalar_one_or_none()
                if not cam or not cam.rtsp_url:
                    logger.warning(f'调度录制: 摄像头 {mac} 不存在或无 RTSP URL')
                    return
                if cam.is_recording:
                    logger.info(f'调度录制: {mac} 已在录制中，跳过')
                    return
            ...
            try:
                ...
                async with AsyncSessionLocal() as _db:
                    rec_db = (
                        await _db.execute(select(Recording).where(Recording.id == rec_id))
                    ).scalar_one_or_none()
                    if rec_db:
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        status_code=500,
        content={
            'error': {'code': 'INTERNAL_ERROR', 'message': '服务器内部错误', 'detail': str(exc)}
        },
    )
