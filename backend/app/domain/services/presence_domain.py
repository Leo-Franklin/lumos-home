...
class PresenceDomainService:
    ...
    async def auto_start_recording(self, camera_mac: str) -> None:
        ...
        async with AsyncSessionLocal() as db:
            cam = (
                await db.execute(select(CameraModel).where(CameraModel.device_mac == camera_mac))
            ).scalar_one_or_none()
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                rec_db = (
                    await db.execute(select(RecordingModel).where(RecordingModel.id == rec_id))
                ).scalar_one_or_none()
                if rec_db:
        ...
        async with AsyncSessionLocal() as db:
            ...
            rec = (
                await db.execute(
                    ...
                    .where(
                        RecordingModel.camera_mac == camera_mac,
            ...
            if rec:
                ...
                # 使用数据库中原始的 started_at（task.started_at 在 stop_recording 后已不可用）
                if rec.started_at:
