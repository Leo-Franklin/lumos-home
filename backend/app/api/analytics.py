...
async def online_trend(
    ...
    subq = (
        select(
            ...
            func.sum(case((DeviceOnlineLog.online_count > 0, 1), else_=0)).label('online_count'),
        )
    ...
    return {
        'data': [
            {'timestamp': f'{row.date}T00:00:00', 'count': round(row.avg_count)} for row in result
        ]
    }
...
async def type_activity(
    ...
    pivot: dict[int, dict[str, float]] = {h: dict.fromkeys(_KNOWN_TYPES, 0.0) for h in range(24)}
    for row in result:
        if row.device_type in _KNOWN_TYPES:
            pivot[row.hour][row.device_type] = round(row.fraction or 0.0, 3)

    return {'data': [{'hour': h, **fractions} for h, fractions in sorted(pivot.items())]}
