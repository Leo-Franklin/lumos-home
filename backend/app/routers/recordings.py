...
from app.deps import CurrentUser, DBDep, StreamUser
    ...
    if settings.nas_mode == 'local':
        ...
        if file_path.startswith('\\\\'):
            ...
            if len(parts) >= 2:
                ...
                nas_access_url = f'smb://{host}/{share}/{rest.replace(chr(92), "/")}'.rstrip('/')
            else:
                nas_access_url = None
            ...
            if settings.nas_smb_host and settings.nas_smb_share:
                ...
                nas_access_url = (
                    f'smb://{settings.nas_smb_host}/{settings.nas_smb_share}/{rel}'.rstrip('/')
                )
            else:
                nas_access_url = None
        else:
...
async def get_recording(recording_id: int, db: DBDep, _: CurrentUser):
    ...
    settings = get_settings()
    storage_type, nas_access_url, file_name = _compute_recording_extra(
        recording.file_path, settings
    )
    return RecordingOut(
        id=recording.id,
