...
async def seeded_client(mem_db, monkeypatch):
    ...
    async with mem_db() as db:
        ...
        await db.commit()
        db.add(
            Camera(device_mac='AA:BB:CC:DD:EE:01', onvif_host='192.168.1.10', rtsp_url='rtsp://x')
        )
        await db.commit()
        db.add_all(
            [
                Recording(
                    ...
                    file_path='/f1',
                    started_at=day0.replace(hour=10, minute=0),
                    status='completed',
                ),
                Recording(
                    camera_mac='AA:BB:CC:DD:EE:01',
                    file_path='/f2',
                    started_at=day0.replace(hour=12, minute=0),
                    status='completed',
                ),
                Recording(
                    camera_mac='AA:BB:CC:DD:EE:01',
                    file_path='/f3',
                    started_at=day1.replace(hour=9, minute=0),
                    status='completed',
                ),
            ]
        )
    ...
    async with mem_db() as db:
        db.add_all(
            [
                ...
                Device(
                    mac='AA:BB:CC:DD:EE:02', device_type='phone', hostname='ph1', is_online=True
                ),
            ]
        )
        await db.commit()
