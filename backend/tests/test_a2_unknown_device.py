...
def test_new_mac_not_in_member_devices_is_unknown():
    enriched = [
        {'mac': 'AA:BB:CC:DD:EE:FF', 'ip': '192.168.1.100', 'vendor': 'Apple', 'hostname': None}
    ]
...
def test_recently_seen_unknown_mac_is_suppressed():
    ...
    result = _find_unknown_devices(
        enriched, original_last_seen, bound_macs, now, staleness_hours=24
    )
