...
function renderChart() {
  ...
  const colorScale = d3
    ...
    .domain([0, maxCount])
    .interpolator(
      d3.interpolateRgb(baseColor, props.color.startsWith('var(') ? '#6366F1' : props.color),
    )

  const svg = d3.select(svgRef.value).append('svg').attr('width', W).attr('height', props.height)
    ...
    svg
      ...
      .on('mousemove', (event) => {
        ...
        tooltip
          ...
          .html(
            `<strong>${dateStr}</strong><br>${t('charts.calendar.recordings', { count: count })}${dur}`,
          )
      })
